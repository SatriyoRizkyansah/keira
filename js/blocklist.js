class BlockList {
  ...
  addBlock_(centerX, centerY, spriteKey, spriteAnimationKey, hysteresis, canMoveOffGrid) {
    const blockState = new BlockState(this.scene_, this.world_, this.grid_, this.directorState_, centerX, centerY, spriteKey, spriteAnimationKey, hysteresis, canMoveOffGrid);
  ...
  addBlockOffGrid(tileX, tileY, spriteKey, spriteAnimationKey) {
    ...
    return this.addBlock_(center.x, center.y, spriteKey, spriteAnimationKey, Config.BLOCK_OFF_GRID_HYSTERESIS, true /* canMoveOffGrid */);
  }

  addBlockInGrid(tileX, tileY, spriteKey) {
    const center = this.grid_.getTileCenter(tileX, tileY);
    return this.addBlock_(center.x, center.y, spriteKey, null /* spriteAnimationKey */, Config.BLOCK_HYSTERESIS, false /* canMoveOffGrid */);
  }
  ...
  update(pattySprite, cursors) {
    ...
    this.sprites_.forEach((blockSprite) => {
      const intersection = Phaser.Geom.Rectangle.Intersection(pattySprite.getBounds(), blockSprite.getBounds());
