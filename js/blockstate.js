class BlockState {
  constructor(scene, world, grid, directorState, centerX, centerY, spriteKey, spriteAnimationKey, hysteresis, canMoveOffGrid) {
  ...
  moveAwayFrom_() {
    ...
    if (!this.shouldMoveTo_(newCenterX, newCenterY, Config.GRID_TILE_SIZE_PX - Config.BLOCK_MOVE_OBSTACLE_ALLOWANCE, Config.GRID_TILE_SIZE_PX - Config.BLOCK_MOVE_OBSTACLE_ALLOWANCE)) {
      // Abort the move.
      return;
    }
  ...
  shouldMoveTo_(newCenterX, newCenterY, width, height) {
    ...
    if (this.world_.anyObstacleInRegion(newCenterX, newCenterY, width, height)) {
