class VictoryCutscene {
  ...
  update() {
    ...
    const distance = Math.abs(pattySprite.x - giftSprite.x) + Math.abs(pattySprite.y - giftSprite.y);
  ...
  pulsateCathyText_() {
    ...
    alphaUpTween.setCallback(
      ...
      () => {
        ...
        alphaDownTween.setCallback("onComplete", () => this.pulsateCathyText_(), [] /* params */);
      },
      [] /* params */
    );
  }
  ...
  spawnRandomConfetti_() {
    ...
    // Do it again!
    setTimeout(() => this.spawnRandomConfetti_(), Config.VICTORY_CONFETTI_FREQUENCY);
  }
}
