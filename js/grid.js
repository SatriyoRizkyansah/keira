class Grid {
  ...
  reset(startY, endY, targetTile) {
    ...
    const fireplace = this.scene_.physics.add.sprite(Config.WORLD_WALL_SIDE_WIDTH_PX + fireplaceWidth / 2, fireplaceCenter.y + Config.GRID_TILE_SIZE_PX / 2 - fireplaceHeight / 2, "fireplace");
  ...
  isInGrid(x, y) {
    return (
      x >= Config.GRID_TOP_LEFT_X && x <= Config.GRID_TOP_LEFT_X + Config.GRID_TILE_SIZE_PX * Config.GRID_WIDTH_IN_TILES && y >= Config.GRID_TOP_LEFT_Y && y <= Config.GRID_TOP_LEFT_Y + Config.GRID_TILE_SIZE_PX * Config.GRID_HEIGHT_IN_TILES
    );
  }
}
