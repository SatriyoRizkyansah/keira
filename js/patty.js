class Patty {
  ...
  update() {
    ...
    if (Math.abs(this.sprite_.x - this.justin_.x) + Math.abs(this.sprite_.y - this.justin_.y) < Config.PATTY_JUSTIN_HEART_PROXIMITY) {
      this.heartTimer_++;
    } else {
      this.heartTimer_ = 0;
    }
    if (this.heartTimer_ >= Config.PATTY_HEART_PERIOD) {
      ...
      const heartSpriteCenter = {
        ...
        y: this.sprite_.y - this.sprite_.displayHeight / 2 + randY - Config.PATTY_HEART_SPREAD,
      };
